...
// Session ID from command line or default
const TARGET_SESSION = process.argv[2] || 'lobby_1333718897';

console.log(`\n🎯 Target Session: ${TARGET_SESSION}\n`);

// Test data - All in the same session
const testClients = [
...
// Clear session messages so old bot messages don't appear again
...
// Verify that session messages have been cleared (debug/test)
...
async function testDisconnect(client) {
    try {
        ...
        // If session already removed (e.g., on first disconnect), ignore 404
...
async function runTests() {
    ...
    // Clear old messages from previous runs
    await clearSessionMessages(TARGET_SESSION);
    await sleep(300);
    
    // Test 2: Connect all test bots to active session
    ...
    // Test 5: Interactive chat simulation
    ...
    // Keep bots connected, then disconnect automatically
    await sleep(1000);
    ...
    // Wait a bit so clients can fetch disconnect messages
    await sleep(5000);

    // Delete session history after test run so bot messages don't appear anymore
    await clearSessionMessages(TARGET_SESSION);

    // Verify that there are no messages left in the session
