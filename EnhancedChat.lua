...
print("EnhancedChat: Loading Natives...")
...
print("EnhancedChat: Loading Modules...")
...
print("EnhancedChat: Modules successfully loaded")

if Settings then
    Settings.LoadSettings()
    print("EnhancedChat: Settings initialized")
end

if GUI and Service then
    GUI.SetService(Service)
    print("EnhancedChat: Service linked with GUI")
end
...
local function OnPresent()
    ...
    pcall(function()
        ...
        if GUI.showSettings and Settings then
            Settings.Render()
        end
        ...
        if Service then
            ...
            if GUI.sessionId and GUI.sessionId ~= currentSessionId then
                print("[EnhancedChat] Session changed!")
                print("  Old: " .. tostring(GUI.sessionId))
                print("  New: " .. tostring(currentSessionId))
...
local function OnScriptStop()
    ...
    print("EnhancedChat: Stopping script...")
    
    if GUI then
        GUI.isVisible = false
        GUI.AddMessage("System", "[DISCONNECT] " .. tostring(GUI.rockstarId) .. " left the chat")
    end
    ...
    pcall(function()
        if Service then
            ...
            print("RockstarID: " .. tostring(rockstarId))
...
chatToggleFeature:AddHotKey(0x78)
